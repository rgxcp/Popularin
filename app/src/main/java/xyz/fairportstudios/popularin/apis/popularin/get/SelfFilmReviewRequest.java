...
public class SelfFilmReviewRequest {
    private Context mContext;
    private int mFilmID;

    public SelfFilmReviewRequest(Context context, int filmID) {
        mContext = context;
        mFilmID = filmID;
    }
    ...
    public void sendRequest(int page, final Callback callback) {
        String requestURL = PopularinAPI.FILM + mFilmID + "/reviews/self?page=" + page;

        JsonObjectRequest selfFilmReview = new JsonObjectRequest(Request.Method.GET, requestURL, null, new Response.Listener<JSONObject>() {
            @Override
        ...
        Volley.newRequestQueue(mContext).add(selfFilmReview);
    }
}
